		...
		primary_action(values) {
			if (values.interval_unit === "month" || values.interval_unit === "months") {
				const selected = document.querySelector(
					'input[name="monthly_repeat_type"]:checked'
				);
			...
			if (values.interval_unit === "year") {
				const selected = document.querySelector(
					'input[name="yearly_repeat_type"]:checked'
				);
				const startDate = frappe.datetime.str_to_obj(values.start_date);
				const weekdayInfo = getWeekdayOccurrenceFor(startDate);
			...
			const selected = [...document.querySelectorAll(".weekday-btn.active")].map((btn) =>
	...
	function updateWeekdayButtons() {
		...
		if (unit === "day" || unit === "days") {
			...
			document.querySelector(
				'#monthly-options input[value="fourth"]'
			).nextSibling.textContent = ` On the ${ordinal} ${weekdayName}`;
			...
			document.querySelector(
				'#yearly-options input[value="fourth"]'
			).nextSibling.textContent = ` On the ${ordinal} ${weekday} of ${monthName}`;
		} else {
...
function editRepeatDialog(frm, options = {}) {
	...
	let repeat_dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			...
			frm.doc.repeat_interval = values?.repeat_interval;
			frm.doc.interval_unit = values?.interval_unit;
			frm.doc.repeat_days = JSON.stringify(values?.repeat_days);
			frm.doc.month_repeat_type = values?.monthly_repeat_type;
			frm.doc.year_repeat_type = values?.yearly_repeat_type;

			if (!frm.doc.subject) {
				frm.doc.subject = "Subject";
			}
			frm.save();
            setTimeout(500);
			frappe.msgprint("Recurring settings saved successfully");
		},
	});
	...
	setTimeout(() => {
		...
		if (options.repeat_interval)
			repeat_dialog.set_value("repeat_interval", options.repeat_interval);
		...
		if (options.monthly_repeat_type) {
			...
			if (options.interval_unit === "month") {
				document
					.querySelector(
						`input[name="monthly_repeat_type"][value="${optionMonthly.type}"]`
					)
					?.click();
			}
		}
		if (options.yearly_repeat_type) {
			...
			if (options.interval_unit === "year") {
				document
					.querySelector(
						`input[name="yearly_repeat_type"][value="${optionYearly.type}"]`
					)
					?.click();
			}
		}
